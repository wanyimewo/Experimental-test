...
class CascadeTransformer(nn.Module):
    def __init__(self, emb_dim, max_seq_len=200):
        super().__init__()
        ...
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=emb_dim,  # 输入特征维度
            nhead=num_heads,  # 多头注意力机制的头数
            batch_first=True  # 输入数据的第一个维度是 batch size
        )
        self.transformer = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)

        # 固定的正弦和余弦位置编码
        self.positional_encoding = self.generate_positional_encoding(max_seq_len, emb_dim)  # (1, max_seq_len, emb_dim)

    def generate_positional_encoding(self, max_seq_len, emb_dim):
        """
        生成固定的正弦和余弦位置编码
        :param max_seq_len: 序列的最大长度
        :param emb_dim: 嵌入维度
        :return: 位置编码张量 (1, max_seq_len, emb_dim)
        """
        ...
        positional_encoding = torch.zeros(max_seq_len, emb_dim)  # (max_seq_len, emb_dim)
        positional_encoding[:, 0::2] = torch.sin(position * div_term)  # 偶数位置使用 sin
        positional_encoding[:, 1::2] = torch.cos(position * div_term)  # 奇数位置使用 cos
        return positional_encoding.unsqueeze(0)  # (1, max_seq_len, emb_dim)

    def generate_causal_mask(self, seq_len):
        """生成因果掩码"""
        mask = causal_mask = torch.triu(torch.ones(seq_len, seq_len), diagonal=1)  # 上三角矩阵
        mask = mask.masked_fill(mask == 1, float('-inf'))  # 将1替换为-inf
        return mask

    def lookup_embedding(self, examples, embeddings):
        """
        根据 examples 查找对应的 embedding
        :param examples: 级联序列 (batch_size, seq_len)
        :param embeddings: 用户 embedding (user_size, emb_dim)
        :return: 查找后的 embedding (batch_size, seq_len, emb_dim)
        """
        output_embedding = []
        for example in examples:
            index = example.clone().detach()
    ...
    def forward(self, examples, user_cas_embedding):
        """
        :param examples: tensor 级联序列 (batch_size, seq_len)
        :param user_cas_embedding: tensor 动态级联图中的用户 embedding (user_size, emb_dim)
        :return: 输出 embedding (batch_size, seq_len, emb_dim)
        """
        # 查找嵌入向量
        cas_embedding = self.lookup_embedding(examples, user_cas_embedding)  # (batch_size, seq_len, emb_dim)

        # 添加位置编码
        seq_len = cas_embedding.size(1)  # 获取当前序列的实际长度
        positional_encoding = self.positional_encoding[:, :seq_len, :]  # 截取有效位置编码
        positional_encoding = positional_encoding.to(cas_embedding.device)
        cas_embedding = cas_embedding + positional_encoding  # 添加位置编码
        causal_mask = self.generate_causal_mask(seq_len)
        causal_mask = causal_mask.to(cas_embedding.device)
        # 输入 Transformer 编码器
        output_embedding = self.transformer(cas_embedding, mask = causal_mask)  # (batch_size, seq_len, emb_dim)

        return output_embedding
#deepseek实现版
# class RelationTransformer(nn.Module):
#     '''Transformer：对从社交图学到的用户embedding做Transformer编码'''
...
#
#     def lookup_embedding(self, examples, embeddings):
#         '''根据examples从embeddings中查找对应的embedding'''
#         output_embedding = []
#         for example in examples:
#             index = example.clone().detach()
#             temp = torch.index_select(embeddings, dim=0, index=index)
#             output_embedding.append(temp)
#         output_embedding = torch.stack(output_embedding, 0)  # (batch_size, seq_len, embed_dim)
#         return output_embedding
#
#     def forward(self, examples, user_social_embedding):
#         '''
#         :param examples: tensor 级联序列 (batch_size, seq_len)
#         :param user_social_embedding: tensor 用户社交embedding (user_size, embed_dim)
#         :return: tensor Transformer编码后的输出 (batch_size, seq_len, embed_dim)
#         '''
#         # 查找用户嵌入
#         user_embedding = self.lookup_embedding(examples, user_social_embedding)  # (batch_size, seq_len, embed_dim)
#
#         # 添加位置编码
#         seq_len = user_embedding.size(1)  # 获取当前序列的实际长度
#         positional_encoding = self.positional_encoding[:, :seq_len, :]  # 截取有效位置编码
#         user_embedding = user_embedding + positional_encoding.to(user_embedding.device)  # 添加位置编码
#
#         # Transformer编码
#         output_embedding = self.transformer_encoder(user_embedding,is_causal=True)  # (batch_size, seq_len, embed_dim)
#         return output_embedding
class RelationTransformer(nn.Module):
    def __init__(self, emb_dim, max_seq_len=200):
        super().__init__()
        ...
        self.relationtransformer = nn.TransformerEncoder(encoder_layer, num_layers=num_layers)
        self.positional_encoding = self.generate_positional_encoding(max_seq_len, emb_dim)

    def generate_positional_encoding(self, max_seq_len, emb_dim):
        position = torch.arange(max_seq_len).unsqueeze(1)
        div_term = torch.exp(torch.arange(0, emb_dim, 2) * (-math.log(10000.0) / emb_dim))
        positional_encoding = torch.zeros(max_seq_len, emb_dim)
    ...
    def forward(self, examples, user_social_embedding):
        ...
        seq_len = user_embedding.size(1)
        positional_encoding = self.positional_encoding[:, :seq_len, :]  # 截取有效位置编码
        positional_encoding = positional_encoding.to(user_embedding.device)
        user_embedding = user_embedding + positional_encoding  # 添加位置编码
        output_embedding = self.relationtransformer(user_embedding)
        return output_embedding
# class CascadeTransformer(nn.Module):
...
#
#         # 输入 Transformer 编码器
#         output_embedding = self.transformer(cas_embedding)  # (batch_size, seq_len, emb_dim)
#
#         return output_embedding
...
#         super().__init__()
#         self.emb_dim = emb_dim
#         # 定义 RNN 层，输入和输出的维度都是 emb_dim，batch_first=True 表示输入的第一个维度是 batch size
#         self.rnn = nn.RNN(self.emb_dim, self.emb_dim, num_layers=1, batch_first=True)
#
#     def lookup_embedding(self, examples, embeddings):
#         """
#         从 embeddings 中查找 examples 对应的嵌入向量
#         :param examples: 级联序列 (batch_size, sequence_length)
#         :param embeddings: 用户嵌入矩阵 (user_size, emb_dim)
#         :return: 查找后的嵌入向量 (batch_size, sequence_length, emb_dim)
#         """
#         output_embedding = []
#         for example in examples:
#             index = example.clone().detach()  # 避免修改原始数据
#             temp = torch.index_select(embeddings, dim=0, index=index)  # 从 embeddings 中选择对应的嵌入向量
#             output_embedding.append(temp)
#         output_embedding = torch.stack(output_embedding, 0)  # 将列表中的张量堆叠成一个新的张量
#         return output_embedding
    ...
    #     前向传播
    #     :param examples: 级联序列 (batch_size, sequence_length)
    #     :param user_cas_embedding: 用户嵌入矩阵 (user_size, emb_dim)
    #     :return: RNN 的输出 (batch_size, sequence_length, emb_dim)
    #     """
    #     # 查找 examples 对应的嵌入向量
    #     cas_embedding = self.lookup_embedding(examples, user_cas_embedding)
    #     # 将嵌入向量输入到 RNN 中
    #     # output_embedding: (batch_size, sequence_length, emb_dim)
    #     # h_n: (1, batch_size, emb_dim) 最后一个时间步的隐藏状态
    #     output_embedding, h_n = self.rnn(cas_embedding)
    #     return output_embedding
...
class Module(nn.Module):
    def __init__(self, user_size, embed_dim, step_split=8, max_seq_len=200, task_num=2, device=torch.device('cuda')):
        ...
        self.relationTransformer = RelationTransformer(self.emb_dim)
        #self.cascadeLSTM = CascadeLSTM(self.emb_dim)
        #self.cascadeRNN = CascadeRNN(self.emb_dim)
        self.cascadeTransformer = CascadeTransformer(self.emb_dim)
    ...
    def forward(self, graph_list, relation_graph, examples):
        ...
        sender_social_embedding = self.relationTransformer(examples,user_social_embedding)
        ...
        #sender_cas_embedding = self.cascadeRNN(examples, user_cas_embedding)
        sender_cas_embedding = self.cascadeTransformer(examples, user_cas_embedding)
